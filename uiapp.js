...
window.executeContextualAreaScanPipeline = function(event) {
    ...
    btnText.textContent = "Scanning area...";
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            btnText.textContent = "Search this area";
        }, 300);
    }, 1000);
};
...
function clearCalculatedRouteLayers() {
    ...
    document.getElementById('traffic-telemetry-node')?.classList.remove('flex');
    ...
    // SYNC UPDATE: Clean optimize fuel stops overlay maps and outputs together
    clearRefuelStrategy();
...
function computeMinimumDistanceToRouteCorridor(sLat, sLon) {
    let minimumTrackSeparationMiles = Infinity;
    for (let i = 0; i < plottedRouteCoordinates.length; i++) {
        const distanceEstimate = computeDistanceVectorMiles(sLat, sLon, plottedRouteCoordinates[i][0], plottedRouteCoordinates[i][1]);
...
async function forceReloadRemotePipelineData() {
    try {
        document.getElementById('live-timestamp-label').textContent = "Synchronizing data layers...";
        const res = await fetch('https://raw.githubusercontent.com/vialon/co-op-fuel-prices/main/fuel_prices.json');
        const data = await res.json();
        
        rawGlobalStationsPool = (data.stations || data || []).map(s => {
...
function focusAndHighlightMapMarker(lat, lon) {
    ...
    if (selectedStation) {
        setTimeout(() => {
            openForecourtDetailSheet(selectedStation);
        }, 300);
    }
}
...
function generateCheapestRankingListDeck(pool, fuelVariant) {
    ...
    const validPool = pool.filter(s => s[fuelVariant] && !isNaN(parseFloat(s[fuelVariant])));
    container.innerHTML = '';

    if (validPool.length === 0) {
        container.innerHTML = `<div class="text-center py-4 text-zinc-400 text-xs font-semibold">No data for fuel variant.</div>`;
        return;
    }

    if (activeTabContext === 'route' && plottedRouteCoordinates.length > 0) {
        blockTitle.textContent = "Cheapest Along Corridor";
        
        const sectionSplits = [
            { name: 'Initial Segment (First 25%)', minPct: 0, maxPct: 0.25 },
            { name: 'Midway Corridor (25% - 75%)', minPct: 0.25, maxPct: 0.75 },
            { name: 'Approaching Destination (Final 25%)', minPct: 0.75, maxPct: 1.0 }
        ];

        sectionSplits.forEach(section => {
            const subGroupWrapper = document.createElement('div');
            subGroupWrapper.className = "space-y-1.5 mt-2";
            subGroupWrapper.innerHTML = `<div class="text-[9px] uppercase font-black tracking-wider text-zinc-400 dark:text-zinc-500 bg-zinc-100 dark:bg-zinc-900 px-2 py-1 rounded">${section.name}</div>`;
            
            let sectionPool = validPool.filter(s => {
                let minTrackDist = Infinity;
                let minTrackIdx = 0;
                for (let i = 0; i < plottedRouteCoordinates.length; i++) {
                    let dist = computeDistanceVectorMiles(parseFloat(s.latitude || s.lat), parseFloat(s.longitude || s.lng), plottedRouteCoordinates[i][0], plottedRouteCoordinates[i][1]);
                    if (dist < minTrackDist) {
                        minTrackDist = dist;
                        minTrackIdx = i;
                    }
                }
                let placementPercentage = minTrackIdx / plottedRouteCoordinates.length;
                return placementPercentage >= section.minPct && placementPercentage <= section.maxPct;
            });

            if (sectionPool.length === 0) {
                subGroupWrapper.innerHTML += `<div class="text-[10px] italic text-zinc-400 p-2 text-center">No coverage in this sector</div>`;
                container.appendChild(subGroupWrapper);
            } else {
                sectionPool.sort((a, b) => parseFloat(a[fuelVariant]) - parseFloat(b[fuelVariant]));
                let slicedTopThree = sectionPool.slice(0, 3).map(station => {
                    let distToAnchor = computeDistanceVectorMiles(mapSearchAnchorCoordinates[0], mapSearchAnchorCoordinates[1], parseFloat(station.latitude || station.lat), parseFloat(station.longitude || station.lng));
                    return { station, distanceToNode: distToAnchor };
                });
        ...
        validPool.slice(0, 3).forEach((station, idx) => {
            ...
            card.innerHTML = `
                <div class="flex items-center gap-2.5 min-w-0">
                    <div class="w-5 h-5 rounded-full bg-emerald-500/10 text-xs flex items-center justify-center font-black text-emerald-500 grow-0 shrink-0">#${idx + 1}</div>
                    <div class="min-w-0">
                        <div class="text-xs font-black text-zinc-900 dark:text-white truncate">${(station.brand_name || 'Independent').replace(/['"]/g, '')}</div>
                        <div class="text-[10px] font-medium text-zinc-400 dark:text-zinc-500 truncate block">${station.address || ''}</div>
                    </div>
                </div>
                <div class="text-right"><div class="text-xs font-black text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded-md border border-emerald-500/20">${val}p</div></div>
            `;
...
function toggleCurrentStationStar(event) {
    ...
    const stationSignature = getStationUniqueSignature(activeSheetStation);
    const index = starredStations.findIndex(s => getStationUniqueSignature(s) === stationSignature);
...
function updateAllStarUIStates() {
    ...
    const stationSignature = getStationUniqueSignature(activeSheetStation);
    const isStarred = starredStations.some(s => getStationUniqueSignature(s) === stationSignature);
    
    btn.innerHTML = isStarred ? ACTIVE_STAR_SVG : INACTIVE_STAR_SVG;
}
...
function renderDirectoryDropdown() {
    ...
    if (activeDirectoryTab === 'stations') {
        ...
        savedRoutes.forEach(route => {
            const cardRow = document.createElement('div');
            cardRow.className = "p-2.5 bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 rounded-xl hover:border-blue-500 transition cursor-pointer shadow-sm flex flex-col justify-between gap-1";
            cardRow.onclick = () => loadSavedRouteCorridorDataIntoWorkspace(route.id);
            cardRow.innerHTML = `
                <div class="flex items-center justify-between w-full min-w-0">
                    <div class="font-black text-xs text-zinc-900 dark:text-white truncate max-w-[170px]">${route.name}</div>
                    <button onclick="deleteSavedRouteCorridor('${route.id}', event)" class="text-[10px] text-zinc-400 hover:text-rose-500 p-1 font-bold">✕</button>
                </div>
                <div class="text-[8px] text-zinc-400 dark:text-zinc-500 truncate">${route.start.split(',')[0]} to ${route.end.split(',')[0]}</div>
            `;
            container.appendChild(cardRow);
        });
    }
...
function assignPricingTierColorStyles(val, type) {
    if(!val || isNaN(parseFloat(val))) return "bg-zinc-50 border-zinc-100 text-zinc-400 dark:bg-zinc-950 dark:border-zinc-900";
    return "bg-emerald-500/5 border-emerald-500/10 text-emerald-600 dark:text-emerald-400";
}
...
function openForecourtDetailSheet(station) {
    ...
    document.getElementById('sheet-brand-title').textContent = (station.brand_name || 'Independent').replace(/['"]/g, '');
    document.getElementById('sheet-brand-sub').textContent = station.brand || 'Fuel Station';
    document.getElementById('sheet-address-string').textContent = station.address || 'UK Highway Vector Network';

    const pricesGrid = document.getElementById('sheet-prices-grid');
    pricesGrid.innerHTML = '';

    const fuelTypesToRender = [
        { name: 'Standard Unleaded (E10)', key: 'E10' },
        { name: 'Super Unleaded (E5)', key: 'E5' },
        { name: 'Standard Diesel (B7)', key: 'B7' },
        { name: 'Premium Diesel', key: 'PremiumDiesel' }
    ];

    fuelTypesToRender.forEach(fuel => {
        const rawVal = station[fuel.key] || station[fuel.key.toLowerCase()] || station.prices?.[fuel.key];
        if (rawVal && !isNaN(parseFloat(rawVal))) {
            const priceBox = document.createElement('div');
            priceBox.className = "p-3 rounded-xl border border-zinc-100 dark:border-zinc-850 bg-zinc-50/50 dark:bg-zinc-950/40 flex items-center justify-between";
            priceBox.innerHTML = `
                <span class="text-xs font-semibold text-zinc-600 dark:text-zinc-400">${fuel.name}</span>
                <span class="text-sm font-black text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-lg border border-emerald-500/15">${parseFloat(rawVal).toFixed(1)}p</span>
                `;
            pricesGrid.appendChild(priceBox);
        }
    });

    updateAllStarUIStates();
    if (window.innerWidth < 768) {
        setMobileSheetUIState('half');
    } else {
...
function getStationUniqueSignature(s) {
    if (!s) return '';
    return `${s.latitude || s.lat}_${s.longitude || s.lng}_${s.brand_name || s.brand || ''}`.replace(/\s+/g, '');
}
...
function bindSwipeGestureDetectionToMobileSheets(handleId, elementId, stateModificationCallback) {
    ...
    targetHandle.addEventListener('touchend', () => {
        const deltaY = touchCurrentY - touchBaseY;
        ...
        if (Math.abs(deltaY) > 40 && timeDiff < 300) {
            if (deltaY > 0) {
                ...
                stateModificationCallback(elementId === 'sidebar' ? 'full' : 'full');
            }
        }
    });
}
...
// Clear all active optimization recommendations, overlay bounds, and layout outputs
...
async function calculateOptimalRefuelStrategy() {
    ...
    // FIXED: Reads visual layout placeholders robustly if left unmodified
    ...
    // Filter candidate stations adjacent to mapped vector track
    ...
    let mappedStations = candidateStations.map(station => {
        ...
        return {
            ...
            price: parseFloat(station[selectedFuelType] || station[selectedFuelType.toLowerCase()] || station.prices?.[selectedFuelType])
        };
    ...
    stopsPlanned.forEach((stop, index) => {
        ...
        // FIXED: Added high zIndexOffset weight parameters to override marker cluster layers
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('clear-route-btn')?.addEventListener('click', clearCalculatedRouteLayers);
